# coding: utf-8

"""
    cPanel UAPI

    UAPI accesses the cPanel interface's features. Use this API to access and modify cPanel account data and settings.

    The version of the OpenAPI document: 11.125.9901.267
    Contact: cs@cpanel.net
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from clientapi_cpanel.models.inline_response200265 import InlineResponse200265

class TestInlineResponse200265(unittest.TestCase):
    """InlineResponse200265 unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> InlineResponse200265:
        """Test InlineResponse200265
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `InlineResponse200265`
        """
        model = InlineResponse200265()
        if include_optional:
            return InlineResponse200265(
                apiversion = 3,
                func = 'export_secret_key',
                module = 'GPG',
                result = clientapi_cpanel.models.inline_response_200_265_result.inline_response_200_265_result(
                    data = clientapi_cpanel.models.inline_response_200_265_result_data.inline_response_200_265_result_data(
                        key_data = '-----BEGIN PGP PRIVATE KEY BLOCK-----
Version: GnuPG v2.0.22 (GNU/Linux)


=W2NA
-----END PGP PRIVATE KEY BLOCK-----', ), 
                    errors = [
                        ''
                        ], 
                    messages = [
                        ''
                        ], 
                    metadata = null, 
                    status = 1, 
                    warnings = [
                        ''
                        ], )
            )
        else:
            return InlineResponse200265(
        )
        """

    def testInlineResponse200265(self):
        """Test InlineResponse200265"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
