# coding: utf-8

"""
    cPanel UAPI

    UAPI accesses the cPanel interface's features. Use this API to access and modify cPanel account data and settings.

    The version of the OpenAPI document: 11.125.9901.267
    Contact: cs@cpanel.net
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from clientapi_cpanel.models.inline_response200264 import InlineResponse200264

class TestInlineResponse200264(unittest.TestCase):
    """InlineResponse200264 unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> InlineResponse200264:
        """Test InlineResponse200264
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `InlineResponse200264`
        """
        model = InlineResponse200264()
        if include_optional:
            return InlineResponse200264(
                apiversion = 3,
                func = 'export_public_key',
                module = 'GPG',
                result = clientapi_cpanel.models.inline_response_200_264_result.inline_response_200_264_result(
                    data = clientapi_cpanel.models.inline_response_200_264_result_data.inline_response_200_264_result_data(
                        key_data = '-----BEGIN PGP PUBLIC KEY BLOCK-----
Version: GnuPG v2.0.22 (GNU/Linux)


=yyST
-----END PGP PUBLIC KEY BLOCK-----
', ), 
                    errors = [
                        ''
                        ], 
                    messages = [
                        ''
                        ], 
                    metadata = null, 
                    status = 1, 
                    warnings = [
                        ''
                        ], )
            )
        else:
            return InlineResponse200264(
        )
        """

    def testInlineResponse200264(self):
        """Test InlineResponse200264"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
